...
const LoginForm = () => {
  ...
  const inputs = () => (
    <>
      <Input
        ...
        value={username}
        classes="basis-full placeholder-gray-400 w-full"
        type="text"
        label="Username"
      />
      {!isUser && (
  ...
  const actionButtons = () => {
    ...
    if (isUser) {
      if (!username || password.firstPassword === "") {
        ...
        return (
          <Button classes="mx-auto flex items-center" primary name={buttonName}>
            {isLoading && <Spinner />}
          </Button>
        );
      }
  ...
  const createErrorMessage = (error) => {
    switch (error) {
      case "noUserFound":
        setErrorMessage(
          <ErrorMessage message="No user found, please try again" />
        );
        return;
      ...
      case "userAlreadyExists":
        setErrorMessage(
          <ErrorMessage message="Username or email already exists, please choose another" />
        );
        return;
      default:
        setErrorMessage(
  ...
  return (
    <form onSubmit={onSubmitHandler}>
      <div>
        ...
        <div className="text-center text-oxford-blue">{loginOptions()}</div>
      </div>
    </form>
  );
};
