...
const setJsx = (text) => {
  switch (text) {
    case "password":
      return (
        <li className={classNames}>Password must be at least 8 characters</li>
      );
    case "lowercase":
      return (
    ...
    case "number":
      return (
        <li className={classNames}>Password must contain at least 1 number</li>
      );
    case "special":
      return (
        <li className={classNames}>
    ...
    case "username":
      return (
        <li className={classNames}>Username must be at least 6 characters</li>
      );
    case "email":
...
export const usePasswordValidation = ({
  ...
  useEffect(() => {
    ...
    setValidLength(null);
    setUpperCase(
      firstPassword.toLowerCase() !== firstPassword ? null : setJsx("lowercase")
    );
    setLowerCase(
      firstPassword.toUpperCase() !== firstPassword ? null : setJsx("uppercase")
    );
    ...
    if (secondPassword) {
      setMatch(
        firstPassword && firstPassword === secondPassword
          ? null
          : setJsx("match")
      );
    }
